...
def to_postgres(data_dir, sl, using_csv=False):
    if using_csv:
        files = glob(data_dir+"/**/*.csv")
        print("Number of Files: {}".format(len(files)))
        outliers = []
        client,meta = connect("postgres", "password", "tes", host="dcos-node2", port=2099)

        try:
            client.execute("select username from users")
